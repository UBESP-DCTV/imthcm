...
fit_hm <- function(events, polluts, data,
    ...
    # aggiungere il fattore `length(pargrid) *` se non si usa
    # **furrr** in `train_each_par`
    pb <- pb_len(length(polluts) * length(events))

    events %>%
        purrr::map(pollut_avg_event_model,
            polluts, par_grid, data, pb
        )
}
